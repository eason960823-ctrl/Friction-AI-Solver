...
st.caption("支援水平面、斜面、拉力夾角等各類題型。請輸入 Gemini API 金鑰並提供題目！")
...
with st.form("solver_form"):
    ...
    # 📷 圖片上傳按鈕
    uploaded_file = st.file_types = ["png", "jpg", "jpeg"]
    image_file = st.file_uploader("上傳題目圖片（支援手寫筆記、考卷截圖、課本拍照）：", type=uploaded_file)

    # 送出按鈕
    submit_button = st.form_submit_with_label = st.form_submit_button(label="開始 AI 精準解題 🚀")

# 5. 當使用者按下送出按鈕後的觸發邏輯
if submit_button:
    # 檢查 API Key
    if not api_key.strip():
        st.error("錯誤：請務必輸入 Gemini API 金鑰！")
    # 檢查是否什麼都沒輸入
        ...
        with st.spinner("AI 老師正在瘋狂受力分析與計算中，請稍候..."):
            try:
                ...
                if image_file:
                    # 讀取上傳圖片的 bytes 並轉成 Gemini 接受的 Part 格式
                ...
                # 6. 輸出結果（Streamlit 原生完美支援 Markdown 與 LaTeX 數學公式渲染！）
                st.success("✨ AI 老師解題分析完成！")
                st.markdown(response.text)

            except Exception as e:
                st.error(f"呼叫 API 時發生錯誤，請確認您的金鑰是否正確。\n詳細原因: {str(e)}")
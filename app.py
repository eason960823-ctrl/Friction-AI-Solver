...
async def read_item(request: Request):
    return templates.TemplateResponse(
        request,
        name="index.html",
        context={"result": None, "question": "", "api_key": ""}
    )
...
async def solve_question(
        request: Request,
        api_key: str = Form(...),  # ✨ 從網頁表單接收使用者動態輸入的金鑰
        question: str = Form(""),  # 文字題目
        image_file: UploadFile = File(None)  # 圖片檔案
):
    try:
        # 安全檢查：確保使用者有輸入金鑰
        if not api_key.strip():
            return templates.TemplateResponse(
                request,
                name="index.html",
                context={"result": "錯誤：請務必輸入 Gemini API 金鑰！", "question": question, "api_key": api_key}
            )

        # 💡 重點改動：不再寫死 Key，而是使用使用者剛剛在網頁填寫的金鑰建立連線
        ...
        if not contents_payload:
            return templates.TemplateResponse(
                request,
                name="index.html",
                context={"result": "請輸入題目文字或上傳題目圖片！", "question": question, "api_key": api_key}
            )
        ...
        result = f"Error: 處理請求或呼叫 Gemini API 時發生錯誤。請確認您的金鑰是否正確。\\n詳細原因: {str(e)}"

    return templates.TemplateResponse(
        request,
        name="index.html",
        context={"result": result, "question": question, "api_key": api_key}
    )
